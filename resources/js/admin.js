try {
  const assetsPath = document.querySelector('meta[name="assets-path"]').getAttribute('content');

  window.tinymce_setup_callback = (editor) => {
    const tinymce = editor;
    tinymce.remove('textarea.richTextBox');
    // tinymce.settings.skin = 'voyager';
  ...
  const bootstrap4Formats = {
    ...
    // Размеры
    bootW25: {
      selector: 'img,p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li,span,a,input,label,button',
      classes: 'w-25',
    },
    bootW50: {
      selector: 'img,p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li,span,a,input,label,button',
